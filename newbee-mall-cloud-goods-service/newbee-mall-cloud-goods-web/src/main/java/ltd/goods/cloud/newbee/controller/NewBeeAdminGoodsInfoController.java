...
public class NewBeeAdminGoodsInfoController {
    ...
    @ApiOperation(value = "根据goodsId查询商品列表", notes = "根据goodsId查询")
    public Result listByGoodsIds(@RequestParam("goodsIds") List<Long> goodsIds) {
        List<NewBeeMallGoods> newBeeMallGoodsByIds = newBeeMallGoodsService.getNewBeeMallGoodsByIds(goodsIds);
        return ResultGenerator.genSuccessResult(newBeeMallGoodsByIds);
    }

    /**
     * 修改商品库存
     * @param updateStockNumDTO
     * @return
     */
