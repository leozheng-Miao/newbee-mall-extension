...
public class NewBeeMallCarouselServiceImpl implements NewBeeMallCarouselService {
    ...
    @Override
    public List<NewBeeMallIndexCarouselVO> getIndexCarouselsForIndex(int number) {
        List<NewBeeMallIndexCarouselVO> newBeeMallIndexConfigGoodsVOS = new ArrayList<>(number);

        List<Carousel> carousels = carouselMapper.findCarouselsByNum(number);
        if (!CollectionUtils.isEmpty(carousels)) {
            newBeeMallIndexConfigGoodsVOS =  BeanUtil.copyList(carousels, NewBeeMallIndexCarouselVO.class);
        }
